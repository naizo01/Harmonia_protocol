...
export default function useAuthenticate(redirectUri?: string) {
  ...
  const { litUserInfo, setLitUserInfo, setAccessToken } = useLitContext();

  useEffect(() => {
    if (authMethod) {
      const fetchUserInfo = async () => {
        try {
          const userInfo = await fetchDiscordUserInfo(authMethod.accessToken); // authMethodからトークンを取得
          setLitUserInfo(userInfo);
        } catch (error) {
   ...
   */
  const authWithStytch = useCallback(async (accessToken: string, userId?: string, method?: string): Promise<void> => {
    setLoading(true);
    setError(undefined);
    setAuthMethod(undefined);

    try {
      const result: AuthMethod = (await authenticateWithStytch(accessToken, userId, method)) as any;
      setAuthMethod(result);
    } catch (err) {
      setError(err);
    } finally {
